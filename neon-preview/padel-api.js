...
(() => {
  ...
  async function signInOrCreateAccount(){const client=await rawClient();const email=(prompt('Neon Admin E-Mail:')||'').trim();if(!email)throw new Error('Anmeldung abgebrochen.');const password=prompt('Neon Admin Passwort:')||'';if(!password)throw new Error('Anmeldung abgebrochen.');let r=await client.auth.signIn.email({email,password});if(r?.error){if(!confirm('Anmeldung fehlgeschlagen. Neues Neon-Admin-Konto anlegen?'))throw new Error(r.error.message||'Anmeldung fehlgeschlagen.');r=await client.auth.signUp.email({email,password,name:'Padel Admin'});}if(r?.error)throw new Error(r.error.message||'Neon Auth Anmeldung fehlgeschlagen.');return true;}
  ...
  window.phNeon={getClient:rawClient,ensureAdmin,isAdmin,claimAdmin,login:signInOrCreateAccount,logout:async()=>{const c=await rawClient();try{await c.auth.signOut()}catch{}adminReady=false}};
