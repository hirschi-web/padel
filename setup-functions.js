...
html += `<div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:6px;"> <div> <label style="font-size:9px; font-weight:700; color:var(--muted); text-transform:uppercase; display:block; margin-bottom:3px;">Team ${teamIdx+1} · Spieler A</label> <input type="text" id="pIdx_${playerAIdx}" value="${valA.replace(/"/g,'&quot;')}" style="margin:0;"> </div> <div> <label style="font-size:9px; font-weight:700; color:var(--muted); text-transform:uppercase; display:block; margin-bottom:3px;">Spieler B</label> <input type="text" id="pIdx_${playerBIdx}" value="${valB.replace(/"/g,'&quot;')}" style="margin:0;"> </div> </div>`;
}
} else {
for(let i = 0; i < count; i++) {
